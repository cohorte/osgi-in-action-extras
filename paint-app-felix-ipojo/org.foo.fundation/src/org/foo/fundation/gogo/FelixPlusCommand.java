 ...
 *
 * @author Thomas Calmant, ogattaz (cohorte-technologies)
 */
@Component(name = "org.foo.fundation.gogo.FelixPlusCommand-factory")
@Instantiate(name = "org.foo.fundation.gogo.FelixPlusCommand")
@Provides(specifications = { IGoGoCommand.class })
public class FelixPlusCommand extends GoGoCommand implements IGoGoCommand {
	 ...
	 */
	public FelixPlusCommand(final BundleContext aContext) {
		super();
		pContext = aContext;
	}
	...
	public void getservice(@Descriptor("the numeric id of the service") final int aServiceID) {
		logInfo("getservice", "execute  getservice(id:%s)", aServiceID);
		...
		try {
			...
			logSevere("service", e, "Error retrieving service by id [%s]", aServiceID);
		}
	 ...
	 *
	 * @param aSpecification Service specification
	 */
	public void getservice(final String aSpecification) {
		logInfo("getservice", "execute  getservice(spec:%s)", aSpecification);
		...
		try {
			...
			logSevere("service", e, "Error retrieving service by spec [%s]", aSpecification);
		}
	}
	 ...
	 *
	 * @param aServiceReference A service reference
	 */
	 ...
	 * standard output
	 *
	 * @param aServiceReferences Some service references
	 */
	 ...
	 * 
	 * @param aSpecification A service specification
	 */
	...
	public void providers(@Descriptor("A service specification") final String aSpecification) {
		logInfo("providers", "execute  providers(sepc:%s)", aSpecification);
		...
		// Print'em
		logInfo("providers", printReferences(serviceReferences));
	}
	...
	public void references(@Descriptor("The specification of the service") final String aSpecification) {
		logInfo("references", "execute  references(spec:%s)", aSpecification);
		try {
			...
			if (serviceReferences == null) {
				...
				for (final ServiceReference<?> svcRef : serviceReferences) {
					...
					logInfo("references", "- reference id:[%4s] Ranking:[%-5s] Specs:%-50s Props:%s",
			...
			logSevere("references", e, "Error printing references");
		}
	}
	...
	public void service(final int aServiceId) {
		logInfo("service", "execute  service(%s)", aServiceId);
		...
		try {
			...
			logSevere("service", e, "Error retrieving service [%s]", aServiceId);
		}
	}
	...
	public void services() {
		logInfo("services", "execute  services()");
		...
		try {
			...
			// Print'em
			logInfo("services", printReferences(serviceReferences));
			logInfo("services", "Found [%d] services", serviceReferences.length);

		} catch (final Exception e) {
			logSevere("services", e, "Error retrieving services");
		}
	}
	...
	public void services(final Bundle aBundle) {
		logInfo("services", "execute  services(bndl:%s)", aBundle.getSymbolicName());
		...
		// Print'em
		logInfo("services", printReferences(serviceReferences));
		logInfo("services", "Bundle [%s] registered [%d] services", aBundle.getSymbolicName(),
