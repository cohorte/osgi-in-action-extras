...
public class ShapeTracker extends ServiceTracker<ISimpleShape, ISimpleShape> {
	...
	private class ShapeRunnable implements Runnable {
		 ...
		 **/
		public ShapeRunnable(int action,
	 ...
	 **/
	@Override
	public ISimpleShape addingService(
			ServiceReference<ISimpleShape> aServiceReference) {

		ISimpleShape wShape = new DefaultShape(pBundleContext,
				aServiceReference);

		// SimpleShape wShape = pBundleContext.getService(ref);

		pLogger.logInfo(this, "addingService", "name=[%s]",
				aServiceReference.getProperty(ISimpleShape.NAME_PROPERTY));
	 ...
	 *            The service object of the corresponding service.
	 **/
	private void processShape(int action,
			ServiceReference<ISimpleShape> aServiceReference,
			ISimpleShape aShape) {
		String name = (String) aServiceReference
				.getProperty(ISimpleShape.NAME_PROPERTY);
	 ...
	 *            The service object of the corresponding service.
	 **/
	private void processShapeOnEventThread(int action,
			ServiceReference<ISimpleShape> aServiceReference,
			ISimpleShape aShape) {
		...
		try {
			if (SwingUtilities.isEventDispatchThread()) {
				...
				SwingUtilities.invokeAndWait(new ShapeRunnable(action,
						aServiceReference, aShape));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
